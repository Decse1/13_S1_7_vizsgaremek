...
module.exports = (app) => {
    ...
    app.post('/api/Beerkezett_rendeles', async (req, res) => {
        try {
            ...
            const [rendelok] = await db.query(`SELECT C.id AS vevo_id, C.nev AS vevo_neve FROM Rendeles R JOIN Partnerseg P ON R.partnerseg = P.id JOIN Ceg C ON P.vevo = C.id JOIN RendelesTetel RT ON R.id = RT.rendeles_id JOIN Termek T ON RT.termek_id = T.id WHERE P.elado = ${cegId} GROUP BY P.vevo;`);
            const rendelesek = [];
    ...
    app.post('/api/Leadott_rendeles', async (req, res) => {
        try {
            const cegId = req.body.cegId;
            const [rendelok] = await db.query(`SELECT C.id AS vevo_id, C.nev AS vevo_neve FROM Rendeles R JOIN Partnerseg P ON R.partnerseg = P.id JOIN Ceg C ON P.vevo = C.id JOIN RendelesTetel RT ON R.id = RT.rendeles_id JOIN Termek T ON RT.termek_id = T.id WHERE P.vevo = ${cegId} GROUP BY P.vevo;`);
            const rendelesek = [];
            for (let i = 0; i < rendelok.length; i++) {
                const vevoId = rendelok[i].vevo_id;
                const [rendeletTermek] = await db.query(`SELECT T.nev AS termek_neve, RT.mennyiseg AS rendelt_mennyiseg FROM Rendeles R JOIN Partnerseg P ON R.partnerseg = P.id JOIN Ceg C ON P.vevo = C.id JOIN RendelesTetel RT ON R.id = RT.rendeles_id JOIN Termek T ON RT.termek_id = T.id WHERE R.status = 'Új' AND P.elado = ${cegId} AND P.vevo = ${vevoId};`);
