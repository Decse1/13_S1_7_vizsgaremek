...
async function ceg_update(ceg) {
    const [rows] = await db.query(
        `UPDATE Ceg SET nev = ?, adoszam = ?, euAdoszam = ?, cim = ?, email = ?, telefon = ?, elofiz = ?, szamla_minta = ?,szamlaszam = ? WHERE id = ?`,
        [ceg.nev, ceg.adoszam, ceg.euAdoszam, ceg.cim, ceg.email, ceg.telefon, ceg.elofiz, ceg.szamla_minta, ceg.szamlaszam, ceg.id]
    );
    return rows;
}
