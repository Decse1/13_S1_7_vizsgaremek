...
async function Partner_en_vevo(ceg_id) {
    const [rows] = await db.query(
        `SELECT Partnerseg.id AS id, Ceg.id AS cId, Ceg.nev AS nev, Ceg.adoszam AS adoszam, 
...
// Ellenőrzi, hogy létezik-e már a partnerség
async function checkPartnerLetezik(eladoId, vevoId) {
    const [rows] = await db.query(
        `SELECT id FROM Partnerseg WHERE elado = ? AND vevo = ?`,
        [eladoId, vevoId]
    );
...
module.exports = (app, authenticateToken) => {
        ...
        try {
            ...
            if (eladoId === vevoId) {
                return res.status(400).json({
                    ok: false,
                    uzenet: "Egy cég nem lehet önmaga partnere!"
                });
            }
            ...
            if (marLetezik) {
                return res.status(409).json({
                    ok: false,
                    uzenet: "Ez a partnerség már létezik!"
                });
            }
            ...
            return res.status(200).json({
                ...
                partnersegId: rows.insertId
            });

        } catch (error) {
            console.error("Hiba partnerség hozzáadásakor:", error);
    ...
    app.post('/api/Partnerek_en_vevo', authenticateToken, async (req, res) => {
        try {
            ...
            if (!ceg_id) {
                return res.status(422).json({ ok: false, uzenet: "Hiányzó cég azonosító" });
            }
            ...
            console.error("Hiba vevő partnerek lekérdezésekor:", err);
            return res.status(500).json({ ok: false, uzenet: "Szerverhiba! Adatbázis hiba." });
        }
    });
    ...
    app.post('/api/Partnerek_en_elado', authenticateToken, async (req, res) => {
        try {
            ...
            console.error("Hiba eladó partnerek lekérdezésekor:", err);
            return res.status(500).json({ ok: false, uzenet: "Szerverhiba! Adatbázis hiba." });
        }
    });
};