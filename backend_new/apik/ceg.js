...
async function ceg_ad(ceg) {
    let fizet = ceg.elofiz ? 1 : 0; // Biztosítjuk, hogy 1 vagy 0 legyen
    
    // ÚJ SÉMA JAVÍTÁS: Bekerült a 'rendeles_minta' is!
...
async function ceg_update(ceg) {
    let fizet = ceg.elofiz ? 1 : 0; // Itt is egységesítve az elofiz kezelése
    
    // ÚJ SÉMA JAVÍTÁS: Bekerült a 'rendeles_minta' is az UPDATE-be!
    const [rows] = await db.query(
        `UPDATE Ceg SET nev = ?, adoszam = ?, euAdoszam = ?, cim = ?, email = ?, telefon = ?, elofiz = ?, szamla_minta = ?, rendeles_minta = ?, szamlaszam = ? WHERE id = ?`,
        [ceg.nev, ceg.adoszam, ceg.euAdoszam, ceg.cim, ceg.email, ceg.telefon, fizet, ceg.szamla_minta, ceg.rendeles_minta, ceg.szamlaszam, ceg.id]
    );
    return rows;
}
...
// Csak egy céget kérdez le adószám alapján (gyorsabb)
...
module.exports = (app, authenticateToken) => {
    ...
    app.post('/api/Ceg_ad', authenticateToken, async (req, res) => {
        try {
            const ceg = req.body;
            // A 'szamlaszam' itt kötelező üzleti logika alapján, ami rendben van.
